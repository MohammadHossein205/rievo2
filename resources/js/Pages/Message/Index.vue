...
const messageData = ref(props.messageData.data);
const links = ref(props.messageData.meta.links);
...
const paginate = ref({
    currentPage: props.messageData.meta.current_page,
    total: props.messageData.meta.total,
    lastPage: props.messageData.meta.last_page,
});
...
const GetData = async (page) => {
    loader.value = true;
    await axios.post(`/admin/get-all-message-data?page=${page}`).then(res => {
...
const searchData = async (page, search) => {
    ...
    await axios.post(`/admin/get-all-message-data?page=${page}`, {search: search}).then(res => {
        paginate.value.currentPage = res.data.current_page;
        messageData.value = res.data.data;
        loader.value = false;
        links.value = res.data.links;
...
const deleteMessage = (id, index) => {
    swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete(`/admin/message/${id}`).then(res => {
                if (res.data === 200) {
...
<template>
    ...
    <div class="bg-[#f8f8f8] w-full h-screen pr-[16rem] pt-[6rem] pl-[.5rem] sm:pr-1">
        ...
        <table class="mt-2 bg-white w-full rounded-lg text-center shadow-xl border-2 border-solid border-slate-100">
            ...
            <tbody>
            <tr v-for="(message,index) in messageData"
                ...
                <td class="flex justify-center items-center gap-1 p-5">
                    <Link :href="`${message.id}/edit`" v-if="can('edit message')">
                        <SvgComponent name="edit-icon" size="1.5" color="#25282f"
                                      class="hover:scale-125"></SvgComponent>
                    </Link>
                    <span @click="deleteMessage(message.id,index)" v-if="can('delete message')"
