...
const {permissionData} = defineProps(['permissionData']);
...
const storeRole = () => {
    if (form.value.permissionIDs.length === 0) {
        swal.fire({
            title: 'دسترسی ها خالی می باشد آیا ادامه می دهید؟',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                axios.post('/admin/role', form.value).then(res => {
                    if (res.data === 200) {
                        swal.fire({
                            title: 'مقام با موفقیت درج شد',
                            icon: 'success',
