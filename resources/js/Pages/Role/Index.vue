...
const roleData = ref(props.roleData.data);
const links = ref(props.roleData.meta.links);
...
const paginate = ref({
    currentPage: props.roleData.meta.current_page,
    total: props.roleData.meta.total,
    lastPage: props.roleData.meta.last_page,
});
...
<template>
    ...
    <div class="bg-[#f8f8f8] w-full h-screen pr-[16rem] pt-[6rem] pl-[.5rem] sm:pr-1">
        ...
        <table class="mt-2 bg-white w-full rounded-lg text-center shadow-xl border-2 border-solid border-slate-100">
            ...
            <tbody>
            <tr v-for="(role,index) in roleData" class="even:bg-slate-100 hover:bg-slate-200 transition-all">
                <td>{{ index + 1 }}</td>
                <td>{{ role.name }}</td>
                <td>{{ role.created_at }}</td>
                <td>{{ role.updated_at }}</td>
                <td class="flex justify-center items-center gap-1 p-5">
                    <Link :href="`${role.id}/edit`" v-if="can('edit role')">
                        <SvgComponent name="edit-icon" size="1.5" color="#25282f"
                                      class="hover:scale-125"></SvgComponent>
                    </Link>
                    <span @click="deleteRole(role.id,index)" v-if="can('delete role')" class="hover:cursor-pointer">
