...
const props = defineProps(['taxData']);
const taxData = ref(props.taxData)
const swal = inject('$swal');
const form = ref({
    tax: taxData.value == null ? '' : taxData.value.tax,
    commission: taxData.value == null ? '' : taxData.value.commission,
});
const errorList = ref([]);

const storeTaxs = () => {
    axios.post('/admin/taxs', form.value).then(res => {
        if (res.data === 200) {
            swal.fire({
...
<template>
    ...
    <div class="bg-[#f8f8f8] w-full h-screen pr-[16rem] pt-[6rem] pl-[.5rem] sm:pr-1">
        <div
            ...
            <h1>اطلاعات مالیات و کمیسیون</h1>
        </div>
        <div class="bg-white rounded-lg mt-1 p-5">
            <div class="w-[100%] mb-5 text-sm font-bold">همه آیتم ها به صورت درصدی هستند</div>
            <div class="grid grid-cols-[auto_auto] justify-items-center gap-5">
                <div class="w-[100%]">
                    <input type="text" placeholder="مالیات را وارد کنید" v-model="form.tax"
                           class="w-[100%] pr-2 rounded-lg border-none bg-slate-100 text-[.85rem]">
                    <div class="text-red-500 pr-1 pt-2" v-if="errorList.tax">
                        {{ errorList.tax[0] }}
                    </div>
                </div>
                <div class="w-[100%]">
                    <input type="text" placeholder="کمیسیون را وارد کنید" v-model="form.commission"
                           class="w-[100%] pr-2 rounded-lg border-none bg-slate-100 text-[.85rem]">
                    <div class="text-red-500 pr-1 pt-2" v-if="errorList.commission">
                        {{ errorList.commission[0] }}
                    </div>
                </div>
            </div>
            <div class="grid grid-rows-2">
                <button class="bg-[#3661ec] p-3 text-white rounded-lg w-36 mt-5 hover:text-red-500 transition-all"
                        @click.prevent="storeTaxs">
                    افزودن
                </button>
