...
const GetData = async (page, search) => {
    loader.value = true;
    await axios.post(`/admin/get-all-dam-data?page=${page}`, {search: search}).then(res => {
...
const deleteMobile = (id, index) => {
    swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete(`/admin/dam/${id}`).then(res => {
                if (res.data === 200) {
...
<template>
    ...
    <div class="bg-[#f8f8f8] w-full h-screen pr-[16rem] pt-[6rem] pl-[.5rem] sm:pr-1 sm:overflow-scroll">
        ...
        <div class="mt-4 flex justify-end items-center">
            ...
            <a href="damvizhe/index" v-if="can('create dam')"
               class="bg-blue-600 px-10 py-3 ml-2 rounded-lg text-white hover:text-slate-900">
                دام ویژه
            </a>
        ...
        <table class="mt-2 bg-white w-full rounded-lg text-center shadow-xl border-2 border-solid border-slate-100">
            ...
            <tr v-for="(dam,index) in damData" class="even:bg-slate-100 hover:bg-slate-200 transition-all">
                ...
                <td>{{ dam.weight }}</td>
                <td>{{ dam.color }}</td>
                <td>{{ dam.ageYear + ' ' + dam.ageMonth }}</td>
                <td>{{ dam.gender }}</td>
                <td>{{ dam.haveMilk }}</td>
                <td>{{ dam.milk_amount }}</td>
