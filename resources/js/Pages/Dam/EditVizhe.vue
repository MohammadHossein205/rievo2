...
const {damData, groupData, groupcompanyData} = defineProps(['damData', 'groupData', 'groupcompanyData']);

const groupCompanyData = ref(groupcompanyData.data);

const HaveMilk = ref(true);

const swal = inject('$swal');

const form = ref({
    group_id: damData.data.group_id,
    group_company_id: damData.data.group_company_id,
    name: damData.data.name,
    slug: damData.data.slug,
    code: damData.data.code,
    price: damData.data.price,
    weight: damData.data.weight,
    color: damData.data.color,
    colorenglish: damData.data.colorenglish,
    ageYear: damData.data.ageYear,
    ageMonth: damData.data.ageMonth,
    gender: damData.data.gender,
    haveMilk: damData.data.haveMilk == 1,
    milk_amount: damData.data.milk_amount,
    description: damData.data.description,
    disount_price: damData.data.disount_price,
    disount_time: damData.data.disount_time,
    imageIds: [],
});
const errorList = ref([]);
...
const updateDam = () => {
    ...
    axios.put(`/admin/damvizhe/${damData.data.id}`, form.value).then(res => {
        if (res.data === 200) {
            swal.fire({
                title: 'دام با موفقیت ویرایش شد',
                icon: 'success',
                confirmButtonText: 'تایید',
...
onMounted(() => {
    damData.data.image.map((image) => {
        MultiSelectValue.value.push(image.url)
        selectIdValue.value.push(image.id)
    });
});
</script>
...
<template>
    ...
    <div class="bg-[#f8f8f8] w-full h-screen pr-[16rem] pt-[6rem] pl-[.5rem] sm:pr-1">
        ...
        <div class="bg-white rounded-lg mt-3 p-5">
            ...
            <div class="mt-10 grid grid-cols-[auto_auto] justify-items-center gap-5">
                <div class="w-[100%]">
                    <select name="level" v-model="form.group_id" class="w-full bg-slate-100 rounded-lg">
                        <option value="-1">نوع دسته بندی دام</option>
                        <option v-for="group in groupData.data" :value="group.id"
                                @click="GetGroupCompanyData(group.name)">
                            {{ group.name }}
                        </option>
                    </select>
                ...
                <div class="w-[100%]">
                <input type="email" placeholder="رنگ اینگلیسی دام را وارد کنید" v-model="form.colorenglish"
                       class="w-[100%] h-10 pr-2 rounded-lg border-none bg-slate-100 text-[.85rem]">
                <div class="allert text-red-500 pr-1 pt-2" v-if="errorList.colorenglish">
                    {{ errorList.colorenglish[0] }}
                </div>
            </div>
                <div class="w-[100%]">
                ...
                <div class="w-[100%] grid grid-cols-[auto_auto] justify-start items-center gap-5">
                    <div>
                        <input type="checkbox" id="havemilk" v-model="form.haveMilk"
                               class="w-4 h-4 text-blue-600 bg-slate-500 rounded focus:ring-white" :disabled="HaveMilk">
                        <label class="font-medium mr-1" for="havemilk">شیرده</label>
                    </div>
                    <div>
                        <input type="email" placeholder="حجم شیر ( کیلوگرم )" v-model="form.milk_amount"
                ...
                <div class="w-[100%]">
                    <input type="email" placeholder="مبلغ تخفیف ( تومان )" v-model="form.disount_price"
                           class="w-[100%] h-10 pr-2 rounded-lg border-none bg-slate-100 text-[.85rem]">
                    <div class="allert text-red-500 pr-1 pt-2" v-if="errorList.disount_price">
                        {{ errorList.disount_price[0] }}
                    </div>
                </div>
                <div class="w-[100%]">
                    <date-picker v-model="form.disount_time" auto-submit class="w-full" color="#262930"
                                 placeholder="تاریخ پایان تخفیف"/>
                    <div class="text-red-500 pr-1 pt-2" v-if="errorList.disount_time">
                        {{ errorList.disount_time[0] }}
                    </div>
                </div>
            </div>
        ...
        <modal-global-gallery v-if="showModal" class="z-10"></modal-global-gallery>
    </div>
</template>
