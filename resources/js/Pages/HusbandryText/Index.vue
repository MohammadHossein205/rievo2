...
const props = defineProps(['husbandryData']);
const husbandryData = ref(props.husbandryData)
const swal = inject('$swal');
const form = ref({
    title_text: husbandryData.value == null ? '' : husbandryData.value.title_text,
    title_description: husbandryData.value == null ? '' : husbandryData.value.title_description,
});
const errorList = ref([]);

const storeHusbandryText = () => {
    axios.post('/admin/husbandry', form.value).then(res => {
        if (res.data === 200) {
            swal.fire({
