...
const faqData = ref(props.faqData.data);
const links = ref(props.faqData.meta.links);
...
const paginate = ref({
    currentPage: props.faqData.meta.current_page,
    total: props.faqData.meta.total,
    lastPage: props.faqData.meta.last_page,
});
...
const GetData = async (page) => {
    loader.value = true;
    await axios.post(`/admin/get-all-faq-data?page=${page}`).then(res => {
        paginate.value.currentPage = res.data.current_page;
        faqData.value = res.data.data;
        loader.value = false;
...
const deleteFaq = (id, index) => {
    swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete(`/admin/faq/${id}`).then(res => {
                if (res.data === 200) {
