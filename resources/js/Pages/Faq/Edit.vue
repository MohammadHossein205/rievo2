...
const {faqsetting} = defineProps(['faqsetting'])

const swal = inject('$swal');
const form = ref({
    title: faqsetting.title,
    body: faqsetting.body,
});
const errorList = ref([]);

const updatefaq = () => {
    axios.put(`/admin/faq/${faqsetting.id}`, form.value).then(res => {
        if (res.data === 200) {
            swal.fire({
                title: 'سوال با موفقیت ویرایش شد',
                icon: 'success',
    ...
    <div class="bg-[#f8f8f8] w-full h-screen pr-[16rem] pt-[6rem] pl-[.5rem] sm:pr-1">
        ...
        <div class="bg-white rounded-lg mt-1 p-5">
            <div class="mt-16 grid grid-cols-[auto_auto] justify-items-center gap-5">
                ...
                <div class="w-[100%]">
                    <input type="text" placeholder="متن را وارد کنید" v-model="form.body"
                           class="w-[100%] h-10 pr-2 rounded-lg border-none bg-slate-100 text-[.85rem]">
