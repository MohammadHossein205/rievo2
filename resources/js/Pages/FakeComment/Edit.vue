...
const {fakecomment} = defineProps(['fakecomment'])

const swal = inject('$swal');

const form = ref({
    namefamily: fakecomment.namefamily,
    image: fakecomment.image,
    body: fakecomment.body,
});
const errorList = ref([]);

const updateFakeComment = () => {
    form.value.image = selectValue.value;
    axios.put(`/admin/fakecomment/${fakecomment.id}`, form.value).then(res => {
        if (res.data === 200) {
            swal.fire({
                title: 'کامنت فیک با موفقیت ویرایش شد',
                icon: 'success',
...
onMounted(() => {
    selectValue.value = fakecomment.image;
})
//for close menu
checkScreen();
</script>
        ...
        <div class="bg-white rounded-lg mt-1 p-5">
            ...
            <div class="grid grid-rows-2">
                <button class="bg-[#3661ec] p-3 text-white rounded-lg w-36 mt-5 hover:text-red-500 transition-all"
                        @click.prevent="updateFakeComment">
                    ویرایش
                </button>
