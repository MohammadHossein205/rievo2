...
const {chartpriceData, groupData} = defineProps(['chartpriceData', 'groupData'])
const swal = inject('$swal');
const form = ref({
    group_id: chartpriceData.data.group_id,
    price: chartpriceData.data.price,
});
const errorList = ref([]);

const updateChartPrice = () => {
    axios.put(`/admin/chartprice/${chartpriceData.data.id}`, form.value).then(res => {
        if (res.data === 200) {
            swal.fire({
                title: 'قیمت دسته با موفقیت ویرایش شد',
                icon: 'success',
        ...
        <div class="bg-white rounded-lg mt-1 p-5">
            ...
            <div class="grid grid-rows-2">
                <button class="bg-[#3661ec] p-3 text-white rounded-lg w-36 mt-5 hover:text-red-500 transition-all"
                        @click.prevent="updateChartPrice">
                    ویرایش
                </button>
